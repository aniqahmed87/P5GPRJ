...
const schemas={
 ...
 rfp:{title:'RFP Tracker',short:'RFPs',icon:'file',description:'Procurement progress across workstreams.',file:'template_RFP(2).xlsx',fields:[f('name','RFP Name'),f('owner','Owners'),f('status','Status','select',['In Progress','Completed']),f('reference','RFP Ref#'),f('remarks','Remarks','textarea')]},
 milestones:{title:'Key Milestones Tracker',short:'Milestones',icon:'flag',description:'Delivery checkpoints and completion.',file:'template_Milestones(2).xlsx',fields:[f('category','Milestone Category'),f('name','Milestone Name'),f('status','Status','select',['Not Started','In Progress','Completed']),f('completionDate','Completion Date','date'),f('progress','Progress (%)','number')]},
...
function normalize(input){
 ...
 return globalThis.PulseInsights?PulseInsights.migrate(out):out;
}
...
root.PulseModel={schemas,uid,now,clone,defaults,normalize,validDate,active,finished,log,newRecord};
